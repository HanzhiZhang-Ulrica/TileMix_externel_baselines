...
def _ensure_vllm_imported():
    global vllm, ops, AttentionType, FlashAttentionMetadata
    global _VLLM_IMPORT_ERROR, _VLLM_IMPORT_ATTEMPTED
    ...
    try:
        ...
        from vllm.attention.backends.flash_attn import (
            FlashAttentionMetadata as _FlashAttentionMetadata,
        )

        vllm = _vllm
        ops = _ops
        AttentionType = _AttentionType
        FlashAttentionMetadata = _FlashAttentionMetadata
...
def _require_vllm():
    ...
    if vllm is None or ops is None or AttentionType is None or FlashAttentionMetadata is None:
        raise ModuleNotFoundError(
            "Optional FlexPrefill vLLM support requires a working vllm installation. Install and configure vllm before using this path."
...
def patch_llama_attention(model: PreTrainedModel, pattern: str):
    ...
    attention_classes = (LlamaAttention, LlamaFlashAttention2)
    if LlamaSdpaAttention is not None:
        attention_classes = attention_classes + (LlamaSdpaAttention,)
...
def patch_qwen2_attention(model: PreTrainedModel, pattern: str):
    ...
    attention_classes = (Qwen2Attention,)
    if Qwen2FlashAttention2 is not None:
        attention_classes = (Qwen2Attention, Qwen2FlashAttention2)
