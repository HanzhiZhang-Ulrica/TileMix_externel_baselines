...
def triton_block_wise_prefill_attention(
    ...
    assert q_len == k_len
    max_triangular_blocks = total_q_blocks * (total_q_blocks + 1) // 2
    if num_block > max_triangular_blocks:
        warnings.warn(
            f"FlexPrefill selected {num_block} blocks, which exceeds the triangular prefill kernel limit of {max_triangular_blocks}; falling back to full causal attention for this call.",
            stacklevel=2,
        )
        if gqa_interleave:
...
def get_active_blocks(
    ...
    global causal_mask_cache
    mask_key = (last_q.device, block_size)
    causal_mask = causal_mask_cache.get(mask_key)
    if causal_mask is None:
        ...
        causal_mask_cache[mask_key] = causal_mask
    ...
    with torch.cuda.device(q.device):
        ...
        if batch_size > 1:
            ...
            for batch_idx in range(batch_size):
                batch_result = flex_prefill_attention(
                    ...
                    return_computational_ratio=return_computational_ratio,
                )
                if return_computational_ratio:
                    batch_out, batch_ratio = batch_result
            ...
            if return_computational_ratio:
                ratio = float(sum(ratios) / len(ratios)) if ratios else 1.0
                return attn_out, ratio
            return attn_out
