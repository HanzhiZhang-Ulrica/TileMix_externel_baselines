...
def _require_flash_attn_for_dense_decode():
    try:
        import flash_attn  # noqa: F401
    except Exception as exc:
        raise ModuleNotFoundError(
            "Local MInference with kv_type='dense' requires a working flash-attn installation. "
...
class MInference:
    ...
    def patch_model(self, model):
        if self.config.kv_type == "dense" and self.config.attn_type in {
            "minference",
            "tri_mix_minference",
            "a_shape",
            "tri_shape",
            ...
            _require_flash_attn_for_dense_decode()
        ...
        if self.config.kv_type == "leank":
            ...
            if patch_leank is None:
                raise ModuleNotFoundError("Optional MInference LeanK dependencies are not available.")
