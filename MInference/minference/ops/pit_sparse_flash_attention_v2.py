...
convert_vertical_slash_indexes = None
_CUDA_EXTENSION_IMPORT_ERROR = None

try:
    from ..cuda import convert_vertical_slash_indexes
except Exception as exc:
    _CUDA_EXTENSION_IMPORT_ERROR = exc


sparse_attn_func = None
convert_vertical_slash_indexes_opt = None
_FAST_KERNEL_IMPORT_ATTEMPTED = False
_FAST_KERNEL_IMPORT_ERROR = None


def _ensure_fast_sparse_kernels_imported():
    global sparse_attn_func, convert_vertical_slash_indexes_opt
    global _FAST_KERNEL_IMPORT_ATTEMPTED, _FAST_KERNEL_IMPORT_ERROR

    if _FAST_KERNEL_IMPORT_ATTEMPTED:
        return
    _FAST_KERNEL_IMPORT_ATTEMPTED = True

    try:
        from sgl_kernel.sparse_flash_attn import (
            convert_vertical_slash_indexes as _convert_vertical_slash_indexes_opt,
        )
        from sgl_kernel.sparse_flash_attn import sparse_attn_func as _sparse_attn_func

        sparse_attn_func = _sparse_attn_func
        convert_vertical_slash_indexes_opt = _convert_vertical_slash_indexes_opt
        return
    except Exception as exc:
        _FAST_KERNEL_IMPORT_ERROR = exc
    ...
    try:
        ...
        if _FAST_KERNEL_IMPORT_ERROR is None:
            _FAST_KERNEL_IMPORT_ERROR = exc

def _require_local_cuda_extension():
    if convert_vertical_slash_indexes is None:
        raise ImportError(
            "Local MInference CUDA extension 'minference.cuda' is required for this sparse attention path. Rebuild it for the current Python/PyTorch/CUDA stack before using this path."
        ) from _CUDA_EXTENSION_IMPORT_ERROR


def _require_fast_sparse_kernels():
    _ensure_fast_sparse_kernels_imported()
    if sparse_attn_func is None or convert_vertical_slash_indexes_opt is None:
        raise ImportError(
            "Optional fast sparse kernels require SGLang or the vLLM sparse attention stack. Install them before using this path."
        ) from _FAST_KERNEL_IMPORT_ERROR
