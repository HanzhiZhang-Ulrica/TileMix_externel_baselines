...
vllm_ops = None
AttentionType = None
get_num_prefill_decode_query_kv_tokens = None
PagedAttention = None
...
def _ensure_vllm_imported():
    global vllm_ops, AttentionType, get_num_prefill_decode_query_kv_tokens
    global PagedAttention, get_tensor_model_parallel_rank
    global flash_attn_varlen_func, flash_attn_with_kvcache
    global vllm_version, _VLLM_IMPORT_ERROR, _VLLM_IMPORT_ATTEMPTED
    ...
    try:
        ...
        from vllm.attention.backends.abstract import AttentionType as _AttentionType
        from vllm.attention.backends.utils import (
            get_num_prefill_decode_query_kv_tokens as _get_num_prefill_decode_query_kv_tokens,
        )
        from vllm.attention.ops.paged_attn import PagedAttention as _PagedAttention
        from vllm.distributed import (
            get_tensor_model_parallel_rank as _get_tensor_model_parallel_rank,
        )
        ...
        get_num_prefill_decode_query_kv_tokens = _get_num_prefill_decode_query_kv_tokens
        PagedAttention = _PagedAttention
        get_tensor_model_parallel_rank = _get_tensor_model_parallel_rank
...
def flash_attn_func(*args, **kwargs):
    if _flash_attn_func is None:
        raise ModuleNotFoundError(
            "MInference requires a working flash-attn installation for FlashAttention-backed decode. "
            "The local Triton fallback has been disabled for correctness. Build/install flash-attn in the active environment and rerun."
        ) from _FLASH_ATTN_IMPORT_ERROR
    return _flash_attn_func(*args, **kwargs)

last_q = 64
LAST_Q_MASK = None
LAST_Q_MASK_DEVICE = None
ROPE_TYPE = None
SEARCH_MASK = None
...
def _require_vllm():
    ...
    if AttentionType is None or flash_attn_varlen_func is None or flash_attn_with_kvcache is None:
        raise ModuleNotFoundError(
            "Optional MInference vLLM backend requires a working vllm and vllm_flash_attn installation. Install or configure them before using vLLM-specific paths."
        ) from _VLLM_IMPORT_ERROR


def _get_last_q_mask(device):
    global LAST_Q_MASK, LAST_Q_MASK_DEVICE
    device = torch.device(device)
    if LAST_Q_MASK is None or LAST_Q_MASK_DEVICE != device:
        arange = torch.arange(last_q, device=device)
        LAST_Q_MASK = arange[None, None, :, None] >= arange[None, None, None, :]
        LAST_Q_MASK_DEVICE = device
    return LAST_Q_MASK
...
def search_pattern_v2(q, k, v, head):
    ...
    def vertical_and_slash_kernel(q, k, v, vertical_size, slash_size):
        ...
        qk[:, :, :, -last_q:] = torch.where(_get_last_q_mask(q.device), qk[:, :, :, -last_q:], -torch.inf)
        qk = torch.nn.functional.softmax(qk, dim=-1, dtype=torch.float32)
        vertical = qk.sum(-2, keepdim=True)
        vertical[...,:30] = torch.inf
...
def gather_last_q_vertical_slash_topk_v4(self, q, k, v, head_id):
    ...
    def vertical_and_slash_kernel(q, k, v, vertical_size, slash_size):
        ...
        qk[:, :, :, -last_q:] = torch.where(_get_last_q_mask(q.device)[...,-last_q:,-last_q:], qk[:, :, :, -last_q:], -torch.inf)
        qk = torch.nn.functional.softmax(qk, dim=-1, dtype=torch.float32)
        vertical = qk.sum(-2, keepdim=True)
        vertical[...,:30] = torch.inf
        vertical_topk = torch.topk(vertical, vertical_size, -1).indices
    ...
    def vertical_and_slash_kernel_extend(q, k, v, vertical_size, slash_size):
        ...
        qk[:, :, :, -last_q-last_start:-last_start] = torch.where(_get_last_q_mask(q.device)[...,-last_q:,-last_q:], qk[:, :, :, -last_q-last_start:-last_start], -torch.inf)
        qk = torch.nn.functional.softmax(qk, dim=-1, dtype=torch.float32)
        vertical = qk.sum(-2, keepdim=True)
        vertical[...,:30] = torch.inf
    ...
    def vertical_and_slash_kernel(q, k, v, vertical_size, slash_size):
        ...
        if vertical.shape[-1] == 0:
            return flash_attn_func(
                q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), 0.0, softmax_scale=None, causal=q_len != 1
            ).view(q.shape[0], 1, q_len, head_dim)
        vertical[..., : min(30, vertical.shape[-1])] = torch.inf
        vertical_size = min(vertical.shape[-1], max(vertical_size, 30))
        vertical_topk = torch.topk(vertical, vertical_size, -1).indices
        ...
        slash[..., -min(100, slash.shape[-1]):] = torch.inf
        slash_size = min(slash.shape[-1], max(slash_size, 50))
        slash = (q_len - 1) - torch.topk(slash, slash_size, -1).indices
