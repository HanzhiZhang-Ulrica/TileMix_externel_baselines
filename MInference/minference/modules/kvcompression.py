...
def _resolve_cache_obj(method: str):
    if method == "kivi":
        try:
    ...
    if method == "leank":
        ...
        return LeanKCache
    return method_to_cache_obj[method]
...
def prepare_inputs_for_generation_kvcompression(
    ...
    def new_prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        cache_obj: Cache = _resolve_cache_obj(method)
        if use_cache and not isinstance(outputs.get("past_key_values", None), cache_obj):
