...
from .configs.model2path import LEANKPATNS, MODEL2PATH, MODEL_ALIASES, resolve_model_name
...
class MInferenceConfig:
    ...
    def update_config_path(self, config_path: str = None, model_name: str = None):
        ...
        if model_name in MODEL_ALIASES:
            warnings.warn(
                f"MInference does not ship a dedicated sparse pattern for {model_name}; using the bundled pattern for {MODEL_ALIASES[model_name]} as the closest available default. Set config_path explicitly if you have a better pattern."
            )
        assert (
            resolved_model_name in MODEL2PATH
        ), f"The model {model_name} you specified is not supported. You are welcome to add it and open a PR :)"
        return MODEL2PATH[resolved_model_name]
